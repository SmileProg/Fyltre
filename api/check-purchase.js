module.exports = async function handler(req, res) {
  ...
  const resp = await fetch(
    `${SUPA_URL}/rest/v1/purchases?select=id,plan&email=eq.${encodeURIComponent(email)}&active=eq.true&limit=1`,
  ...
  return res.status(200).json({ authorized: !!row, plan: row?.plan || null });
};
